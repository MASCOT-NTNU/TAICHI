...
@jit(nopython=True)
def get_ind_at_location2d_xy(coordinates, x, y):
    dist_x = coordinates[:, 0] - x
    dist_y = coordinates[:, 1] - y
    dist = dist_x ** 2 + dist_y ** 2
    ind = np.argmin(dist)
...
def get_excursion_prob(mu, SigmaDiag, threshold):
  temp = (threshold - mu) * SQRT1_2 / SigmaDiag
...
def checkfolder(folder):
    if not os.path.exists(folder):
        ...
        print(folder + " exists already, no need to create a new one.")
...
def interpolate_3d(x, y, z, value, refill_nan=True):
    '''
