"""
This script will be run onboard one AUV

It will conduct the adaptive sampling.
It is composed of three main phases:
- Phase I: it instructs the agent to move from TBS towards LADE at the starting location of the lawnmower.
- Phase II: it instructs the agent to move adaptively throughout the whole process.
- Phase III: it instructs the agent to move adaptively and share data between the other agent.
...
class AgentAdaptive:
    ...
    def run(self):
        ...
        self.counter_waypoint_data_assimilation = 0
        self.auv_data = []
                ...
                if not self.popup:
                    if (self.auv.auv_handler.getState() == "waiting" and
                        ...
                        if self.prerun_mode:
                            ...
                            self.save_assimilated_data_over_iterations(iter_no=self.counter_waypoint_data_assimilation,
                                                                       ind=ind_assimilated,
    ...
    def save_assimilated_data_over_iterations(self, iter_no, ind, salinity):
        """
        This will save the assimilated data indices and values

        It uses iter_no as the key in the dict, and then expand the key-values pair over iterations.
        It first loads the previous dict and then append new values onto it.
