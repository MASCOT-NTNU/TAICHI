"""
This script simulates Agent Yang's behaviour
Author: Yaolin Ge
...
class Agent2:
    ...
    def run(self):
        ...
        filename_ind_next = FILEPATH + "Simulation/Waypoint/Agent2/ind_next.txt"
            ...
            if i == 0:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                    executor.submit(self.myopic3d_planner.find_next_waypoint_using_min_eibv,
                                    ...
                                    self.ind_visited_waypoint,
                                    filename=filename_ind_next)
                self.ind_next_waypoint = int(np.loadtxt(filename_ind_next))
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                    executor.submit(self.myopic3d_planner.find_next_waypoint_using_min_eibv,
                                    ...
                                    self.ind_visited_waypoint,
                                    filename=filename_ind_next)
                self.ind_pioneer_waypoint = int(np.loadtxt(filename_ind_next))
            else:
                self.myopic3d_planner.update_planner(knowledge=self.knowledge, gmrf_model=self.gmrf_model)
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                    executor.submit(self.myopic3d_planner.find_next_waypoint_using_min_eibv,
